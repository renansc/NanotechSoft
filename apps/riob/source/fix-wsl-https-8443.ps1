...
function Get-WslIp {
  ...
  if (-not $ip) { throw "Nao foi possivel obter o IP do WSL para '$Distro'." }
  return $ip
}

function Ensure-Iphlpsvc {
  $svc = Get-Service -Name iphlpsvc -ErrorAction Stop
  if ($svc.Status -ne "Running") { Start-Service -Name iphlpsvc }
}

function Reset-PortProxy8443 {
  param([string]$WslIp)
...
function Show-Rule8443 {
  Write-Host "`nRegras ativas para 8443:"
  & netsh interface portproxy show v4tov4 | Select-String -Pattern "8443|Endereco|Address|Listen"
}

function Test-Https8443 {
  ...
  try {
    ...
    Write-Host "Confirme se o Flask no WSL esta rodando em HTTPS na porta 8443."
  }
}
...
Reset-PortProxy8443 -WslIp $wslIp
Show-Rule8443
Test-Https8443
