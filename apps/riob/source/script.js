...
function showTab(tabId, el) {
  ...
  if (tabId === "estoque") {
    if (!window.__estoqueView || !["lancar", "posicao", "cadastrar", "rastreio", "estatistica"].includes(window.__estoqueView)) {
...
function setEstoqueView(view){
  const nextView = view === "posicao"
    ? "posicao"
    : (view === "cadastrar" ? "cadastrar" : (view === "rastreio" ? "rastreio" : (view === "estatistica" ? "estatistica" : "lancar")));
  ...
  const viewEstatistica = document.getElementById("estoqueViewEstatistica");
  ...
  const btnEstatistica = document.getElementById("estoqueViewBtnEstatistica");
  ...
  if (viewEstatistica) viewEstatistica.classList.toggle("hidden", nextView !== "estatistica");
  ...
  if (btnEstatistica) btnEstatistica.classList.toggle("active", nextView === "estatistica");
  ...
  } else if (nextView === "estatistica") {
    carregarEstatisticaLoteEstoque().catch((e) => {
      console.warn("estatistica lote erro:", e);
    });
  }
}
...
function _estoqueLoteDataPadrao(){
  const agora = new Date();
...
function _estoqueFmtDateOnlyBr(v){
...
async function carregarEstatisticaLoteEstoque(force = false){
  const dataEl = document.getElementById("estoqueLoteEstatisticaData");
  if (dataEl && !dataEl.value) dataEl.value = _estoqueLoteDataPadrao();
  const data = dataEl?.value || _estoqueLoteDataPadrao();
  const resumoEl = document.getElementById("estoqueLoteEstatisticaResumo");
  if (!force && estoqueState.loteEstatistica?.resumo?.data_lote === data) {
    renderEstatisticaLoteEstoque(estoqueState.loteEstatistica);
    return;
  }
  if (resumoEl) resumoEl.textContent = "Calculando estatistica do lote...";
  const resp = await apiFetch(`/api/estoque/lotes/estatistica?data=${encodeURIComponent(data)}`);
  const payload = await resp.json().catch(() => ({}));
  if (!resp.ok || payload?.erro) {
    const msg = payload?.erro || "Falha ao calcular estatistica do lote.";
    if (resumoEl) resumoEl.textContent = msg;
    throw new Error(msg);
  }
  estoqueState.loteEstatistica = payload || {};
  renderEstatisticaLoteEstoque(payload || {});
}

function renderEstatisticaLoteEstoque(payload = {}){
  const resumo = payload?.resumo || {};
  const resumoEl = document.getElementById("estoqueLoteEstatisticaResumo");
  const cardsEl = document.getElementById("estoqueLoteEstatisticaCards");
  const usoBody = document.getElementById("estoqueLoteEstatisticaUsoBody");
  const vendasBody = document.getElementById("estoqueLoteEstatisticaVendasBody");
  const entradasBody = document.getElementById("estoqueLoteEstatisticaEntradasBody");
  const saidasBody = document.getElementById("estoqueLoteEstatisticaSaidasBody");

  if (resumoEl) {
    resumoEl.textContent = `Lote ${resumo.lote_codigo || "-"} | Fabricacao ${_estoqueFmtDateOnlyBr(resumo.data_lote)} | Saida estimada ${_estoqueFmtDateOnlyBr(resumo.data_saida_estimativa)} | Regra D+${Number(resumo.dias_fabricacao_saida || 2)}`;
  }
  if (cardsEl) {
    cardsEl.innerHTML = _renderCardsVendasResumo([
      ["Entradas", _fmtNumVendas(resumo.notas_entrada || 0)],
      ["Saidas D+2", _fmtNumVendas(resumo.notas_saida || 0)],
      ["Itens uso", _fmtNumVendas(resumo.itens_uso || 0)],
      ["Qtd uso", _estoqueFormatQtd(resumo.qtd_uso_estimado || 0)],
      ["Valor uso", _fmtMoneyVendas(resumo.valor_uso_estimado || 0)],
      ["Produtos venda", _fmtNumVendas(resumo.vendas_produtos || 0)],
      ["Notas venda", _fmtNumVendas(resumo.vendas_notas || 0)],
      ["Venda liquida", _fmtMoneyVendas(resumo.vendas_valor_liquido || 0)],
    ]);
  }

  const usoRows = Array.isArray(payload?.uso_estimado) ? payload.uso_estimado : [];
  if (usoBody) {
    usoBody.innerHTML = usoRows.length ? usoRows.map((item) => `
      <tr>
        <td>${_escHtml(item.uso || "-")}</td>
        <td>${_escHtml(_estoqueGrupoLabel(item.grupo_estoque || "OUTROS"))}</td>
        <td>${_escHtml(item.produto || "-")}${item.origem === "notas_saida_d2" ? ' <span class="hint-chip">D+2</span>' : ""}</td>
        <td>${_escHtml(_estoqueFormatQtd(item.quantidade || 0))}</td>
        <td>R$ ${_escHtml(_fmtMoney(item.valor_total || 0))}</td>
        <td>${_escHtml((Array.isArray(item.notas) ? item.notas : []).join(", ") || "-")}</td>
      </tr>
    `).join("") : `<tr><td colspan="6">Nenhum consumo/saida encontrado para estimar este lote.</td></tr>`;
  }

  const vendasRows = Array.isArray(payload?.vendas?.rows) ? payload.vendas.rows : [];
  if (vendasBody) {
    vendasBody.innerHTML = vendasRows.length ? vendasRows.map((item) => `
      <tr>
        <td>${_escHtml(item.produto || "-")}</td>
        <td>${_escHtml(item.grupo || "-")}</td>
        <td>${_escHtml(_fmtNumVendas(item.notas || 0))}</td>
        <td>${_escHtml(_fmtNumVendas(item.clientes || 0))}</td>
        <td>${_escHtml(_estoqueFormatQtd(item.quantidade || 0))}</td>
        <td>${_escHtml(_estoqueFormatQtd(item.caixas || 0))}</td>
        <td>R$ ${_escHtml(_fmtMoney(item.valor_liquido || 0))}</td>
      </tr>
    `).join("") : `<tr><td colspan="7">${_escHtml(payload?.vendas?.erro || "Nenhuma venda encontrada para a data de saida estimada.")}</td></tr>`;
  }

  const renderNotas = (rows, tipo) => rows.length ? rows.map((item) => `
    <tr>
      <td>${_escHtml(_estoqueFmtDateOnlyBr(item.data_emissao))}</td>
      <td>${_escHtml(item.numero_nota || "-")}</td>
      <td>${_escHtml(tipo === "entrada" ? (item.emitente_nome || "-") : (item.destinatario_nome || "-"))}</td>
      <td>${_escHtml(_fmtNumVendas(item.itens || 0))}</td>
      <td>${_escHtml(_estoqueFormatQtd(item.quantidade || 0))}</td>
      <td>R$ ${_escHtml(_fmtMoney(item.valor_total || 0))}</td>
    </tr>
  `).join("") : `<tr><td colspan="6">Nenhuma nota ${tipo === "entrada" ? "de entrada" : "de saida"} encontrada.</td></tr>`;

  if (entradasBody) {
    entradasBody.innerHTML = renderNotas(Array.isArray(payload?.notas_entrada) ? payload.notas_entrada : [], "entrada");
  }
  if (saidasBody) {
    saidasBody.innerHTML = renderNotas(Array.isArray(payload?.notas_saida) ? payload.notas_saida : [], "saida");
  }
}
