...
async function refreshData(options = {}) {
    ...
    if (state.selectedHostId && !state.hosts.some((host) => host.moid === state.selectedHostId)) {
        state.selectedHostId = null;
        state.selectedHostDetails = null;
    }
...
function formatLicenseRemaining(remainingHours) {
    if (remainingHours == null || Number.isNaN(Number(remainingHours))) {
        return "Horas restantes indisponiveis";
    }

    const totalHours = Math.max(0, Math.floor(Number(remainingHours)));
    const days = Math.floor(totalHours / 24);
    const hours = totalHours % 24;

    if (days > 0 && hours > 0) {
        return `${escapeHtml(String(days))} d ${escapeHtml(String(hours))} h restantes`;
    }
    if (days > 0) {
        return `${escapeHtml(String(days))} d restantes`;
    }
    return `${escapeHtml(String(hours))} h restantes`;
}
