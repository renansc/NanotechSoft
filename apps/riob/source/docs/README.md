...
- no Kanban RioB, cards ativos da mesma rota ou cidade podem ser unidos mesmo quando possuem datas ou status diferentes, pelo botao `Unificar` ou arrastando um card sobre outro; os vinculos operacionais sao transferidos ao destino em uma transacao, o status do destino prevalece e o card de origem fica arquivado no historico
