...
- o modulo `Workflow -> Vendas Diario`, com importacao idempotente dos TXT de Conferencia de Pedidos e PDFs de carga, consulta por data e leitura automatica das pastas montadas `RB_VENDAS_DIARIO_TXT_DIR` e `RB_VENDAS_DIARIO_PDF_DIR` todos os dias no horario `RB_VENDAS_DIARIO_HORA` (08:00 por padrao)
...
- TXT e PDF inequivocos sao unidos automaticamente quando o SELLOUT ativo confirma mesma data, vendedor, cidade, mapa final unico e sufixo equivalente do mapa; casos ambiguos continuam como sugestao para confirmacao explicita, todas as fontes e divergencias sao preservadas; cidade, rota e mapas resolvidos aparecem no card final, e o prefixo do mapa sugere o caminhao cadastrado
- o Workflow de Vendas Diario possui `Listar cargas da semana`: a semana operacional comeca no domingo e termina no sabado (por exemplo, `2026-W33` vai de 09/08 a 15/08); ela e escolhida no calendario e a tabela mostra cada carga PDF, inclusive as ja enviadas ao Kanban, com motorista, entregador, vendedor, caminhao, placa, cidade, rota, peso e quantidade de entregas
- no Kanban RioB, cards ativos do mesmo caminhao podem ser unidos independentemente de data, status, rota, cidade, carga, motorista ou ajudante, pelo botao `Unificar` ou arrastando um card sobre outro; os vinculos operacionais sao transferidos ao destino em uma transacao, o status do destino prevalece e o card de origem fica arquivado no historico; o botao `Desagrupar` desfaz a ultima unificacao em ordem inversa, restaura os cards e devolve os vinculos registrados para a origem
