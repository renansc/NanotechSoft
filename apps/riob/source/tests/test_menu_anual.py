...
# Load only pure report functions; never bootstrap the operational server.
import ast
import os
import types
import datetime
server = types.ModuleType("annual_report_fixture")
server.__file__ = str(Path(__file__).resolve().parents[1] / "server.py")
server.__dict__.update(datetime=datetime, os=os)
server._vendas_obter_cache_ativo = lambda **kw: None
server._vendas_relatorio_base_rows = lambda *args: None
server._vendas_cache_entry_publico = lambda entry: dict(entry)
source = Path(server.__file__).read_text()
functions = {n.name:n for n in ast.parse(source).body if isinstance(n, ast.FunctionDef)}
pending = ["_vendas_comparativo_anual", "_coletar_relatorio_vendas_percentual_vendas_anual", "_vendas_rows_filtradas_base"]
selected = {}
while pending:
    name = pending.pop()
    if name in selected or name in server.__dict__ or name not in functions:
        continue
    node = functions[name]
    selected[name] = node
    pending.extend(n.id for n in ast.walk(node) if isinstance(n, ast.Name) and n.id in functions)
exec(compile(ast.Module(body=list(selected.values()), type_ignores=[]), server.__file__, "exec"), server.__dict__)
