...
function MEDIA_clearPanels(){
  ...
  $("spotifyActions").classList.add("hidden");
...
async function MEDIA_loadSongSource(song){
  ...
  if(source.type === "local"){
    ...
    $("spotifyActions").classList.remove("hidden");
    $("spotifyEmbedFrame").src = source.embedUrl || "";
    $("audioViewStatus").textContent = `Spotify: ${source.label || source.url || "fonte configurada"}. Use o embed abaixo ou abra no app/site do Spotify.`;
  }else{
...
function UI_updatePlaybackButtons(){
  ...
  if(state.activeSourceType === "youtube") {
    ...
    hintText += "Spotify fica disponível como embed para consulta e abertura no app oficial.";
  }
  
  $("playerHint").innerHTML = canTimeline
    ? hintText
    : "Modo slide continua disponível, mas timeline e marcação automática de tempo requerem MP3 local ou YouTube.";
}
...
function UI_renderAuthStatus(){
  ...
  if($("spotifyEditorAuthMessage")) {
    $("spotifyEditorAuthMessage").textContent = spotifyConnected
      ? " - Busca do Spotify pronta para uso no editor."
      : " - Conecte sua conta na aba Config para usar a busca no catálogo com mais praticidade.";
  }
}
...
function bindEvents(){
  ...
  $("openSpotifyExternalBtn").addEventListener("click", () => {
    const song = PLAYER_song();
    const url = song?.audioSource?.type === "spotify" ? song.audioSource.url : "";
    if(!url) return alert("Essa música não possui link válido do Spotify.");
